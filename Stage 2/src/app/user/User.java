...
public class User {
    ...
    private boolean online = true;    // it is online
    @Getter
    @Setter
    private Page page = new HomePage();
    ...
    public String select(final int itemNumber) {
        ...
        if (searchBar.getLastSearchType().equals("artist")) {
            page = new ArtistPage();
            page.setUser(Admin.getInstance().getArtist(selected.getName()));
            return "Successfully selected " + selected.getName() + "'s page.";
        }

        if (searchBar.getLastSearchType().equals("host")) {
            page = new HostPage();
            page.setUser(Admin.getInstance().getHost(selected.getName()));
            return "Successfully selected " + selected.getName() + "'s page.";
        }
    ...
    /**
     *
     * @return message depending on whether the status was changed or not
     */
    public String switchConnectionStatus() {
        if (online) {
            // it is online => switch it to offline
            online = false;
    ...
    public String printCurrentPage() {
        return page.printPage(page.getUser());
    }

    /**
     *
     * @param nextPage next page
     * @return message depending on whether the page was changed or not
     */
    public String changePage(final String nextPage) {
        PageFactory pageFactory = new PageFactory();
        Page newPage = pageFactory.createPage(nextPage);

        if (page == null) {
            return username + " is trying to access a non-existent page.";
        } else {
            page = newPage;
            page.setUser(Admin.getInstance().getUser(username));
